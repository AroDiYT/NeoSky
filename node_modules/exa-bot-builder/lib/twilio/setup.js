...
module.exports = function twilioSetup(api, bot, logError, optionalParser, optionalResponder) {
  ...
  api.addPostDeployStep('twilio', (options, lambdaDetails, utils) => {
    return Promise.resolve().then(() => {
        if (options.twilioAccountId && options.twilioAuthToken && options.twilioSendingNumber) {
          return Promise.resolve({
            TWILIO_ACCOUNT_SID: options.twilioAccountId,
            TWILIO_AUTH_TOKEN: options.twilioAuthToken,
            TWILIO_NUMBER: new Buffer(options.twilioSendingNumber).toString('base64')
          });
        }
      ...
      .then((variables) => {
        return {
          botUrl: `${lambdaDetails.apiUrl}/twilio`,
          variables: variables
        };
      });
  });

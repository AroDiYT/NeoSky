...
module.exports = function fbSetup(api, bot, logError, optionalParser, optionalResponder) {
  ...
  api.get('/facebook', request => {
    let qs = request.queryString || request.query;
    if (qs['hub.verify_token'] === request.env.facebookVerifyToken)
      return Promise.resolve(qs['hub.challenge']);

    logError(`Facebook can't verify the token. It expected '${request.env.facebookVerifyToken}', but got '${qs['hub.verify_token']}'. Make sure you are using the same token you set in 'facebookVerifyToken' stage env variable.`);
    return 'Error';
  }, {
      success: {
    ...
    return Promise.resolve().then(() => {
      if (options.facebookVerifyToken && options.facebookAccessToken &&
        ...
        return Promise.resolve({
          facebookVerifyToken: options.facebookVerifyToken,
          facebookAccessToken: options.facebookAccessToken,
          facebookAppSecret: options.facebookAppSecret
        });
      }
      ...
      .then((vars) => {
        return {
          botUrl: `${lambdaDetails.apiUrl}/facebook`,
          variables: vars
        };
      });
  });

...
module.exports = function slackSetup(api, bot, logError, optionalParser, optionalResponder) {
  ...
  api.addPostDeployStep('slackSlashCommand', (options, lambdaDetails, utils) => {
    return Promise.resolve().then(() => {
        if (options.slackToken && options.slackWebhookToken) {
          return Promise.resolve({
            slackToken: options.slackToken,
            slackWebhookToken: options.slackWebhookToken
          });
        }

        if (options.slackClientId && options.slackClientSecret &&
          options.slackVerificationToken && options.slackHomePageUrl) {
          return Promise.resolve({
            slackClientId: options.slackClientId,
            slackClientSecret: options.slackClientSecret,
            slackVerificationToken: options.slackVerificationToken,
            slackHomePageUrl: options.slackHomePageUrl,
      ...
      .then((variables) => {
        return {
          botUrl: `${lambdaDetails.apiUrl}/slack/slash-command`,
          variables: variables
        };
      });
  });

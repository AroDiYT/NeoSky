...
class QueryModelBase {
    ...
    getPropertiesFromKeys(keys, addIdentity) {
        ...
        if (keys) {
            ...
            if (addIdentity) {
                // for later use
                props.push(...this.metaModel.qmCache.primaryKeyProps.filter((prop) => !addedMap.has(prop.key)));
            }
        }
        else {
    ...
    bindAllInputParams(model, subset, addIdentity) {
        const hostParams = {};
        const props = this.getPropertiesFromKeys(subset, addIdentity);
        props.forEach((prop) => {
            this.setHostParam(hostParams, prop, model);
        });
        return hostParams;
    }
    bindNonPrimaryKeyInputParams(model, subset) {
        const hostParams = {};
        const props = this.getPropertiesFromKeys(subset);
        props
